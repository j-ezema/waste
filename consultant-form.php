...
<head>
  ...
  <meta name="keywords" content="bootstrap, bootstrap4" />
		...
		<title>Waste4Meal - Join as a Consultant Form</title>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $phone = $_POST["phone"];
	    ...
	    // File handling
    $file1_name = $_FILES['cv']['name'];
    $file1_tmp = $_FILES['cv']['tmp_name'];
    move_uploaded_file($file1_tmp, "uploads/" . $file1_name);
	
	    $file2_name = $_FILES['other_documents']['name'];
    $file2_tmp = $_FILES['other_documents']['tmp_name'];
    move_uploaded_file($file2_tmp, "uploads/" . $file2_name);
	
	    $file3_name = $_FILES['id']['name'];
    $file3_tmp = $_FILES['id']['tmp_name'];
    move_uploaded_file($file3_tmp, "uploads/" . $file3_name);

    // Define the path to your Excel file
    $excelFile = 'consultant_form.csv';

    // Check if the file exists, if not, create it and add headers
    if (!file_exists($excelFile)) {
        $excelHeader = ["Surname", "Firstname", "Other names", "Gender","Date of Birth", "Email", "Phone", "CV", "Other documents", "ID"];
    ...
    // Add new form data
    $excelData[] = [$sur_name, $f_name, $o_name, $gender, $dob, $email, $phone, $file1_name, $file2_name, $file3_name];
...
?>
    <form method="post" action="<?php echo $_SERVER["PHP_SELF"]; ?>" enctype="multipart/form-data">
		       ...
		       <label for="cv" class="text-black">CV <span class="text-danger">*</span></label>
		                <input type="file" class="form-control" id="cv" name="cv" required>
		            </div>
					
					   <div class="form-group">
		       <label for="other_documents" class="text-black">Other Documents <span class="text-danger">*</span></label>
